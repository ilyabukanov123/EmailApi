...
func (s *Server) searchPattern(url string) string {

	hitPatterns := make([]string, 0)
	parseUrl := getParseUrl(url)

	for pattern, _ := range s.patterns {
...
func (s *Server) checkPattern(method, pattern string) error {

	if _, ok := s.patterns[pattern][method]; ok {
